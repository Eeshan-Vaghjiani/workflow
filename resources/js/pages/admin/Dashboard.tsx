...
const StatsCard = ({ title, value, icon, change, positive = true, bgColor = "bg-white dark:bg-gray-800", delay = 0 }: StatsCardProps) => {
    return (
        <motion.div
            className={`${bgColor} rounded-xl shadow-sm p-6 border border-gray-100 dark:border-gray-700`}
    ...
    return (
        ...
        >
            <div className="flex items-center justify-between mb-4">
                ...
                <a href="/admin/audit" className="text-sm text-[#00887A] dark:text-[#00ccb4] hover:underline flex items-center">
            ...
            <div className="space-y-4">
                {recentActivities.map((activity) => (
                    <div key={activity.id} className="flex items-start">
                        <div className={`    p-2 rounded-full mr-3 ${activity.status === 'success' ? 'bg-green-100 text-green-600' :
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {quickActions.map((action) => (
                    <motion.a
                        key={action.id}
                        href={action.href}
                        className="flex items-center p-4 border border-gray-100 dark:border-gray-700 rounded-lg hover:bg-[#D3E3FC]/20 dark:hover:bg-gray-700/30 transition-colors"
                ...
                {systemMetrics.map((metric) => (
                    <div key={metric.id} className="space-y-2">
                        <div className="flex justify-between items-center">
                            ...
                            <span className={`text-xs font-medium px-2 py-1 rounded-full ${metric.status === 'good' ? 'bg-green-100 t    ext-green-800 dark:bg-green-900/30 dark:text-green-400' :
                        ...
                        <div cl assName="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                            <div
